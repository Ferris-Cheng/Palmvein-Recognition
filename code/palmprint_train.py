"""
Functions for training palmprint cnn
...
def main(args):
    ...
    palmPrintsNet.write_arguments_to_file(args, os.path.join(log_dir, 'arguments.txt'))#将所有的参数存入arguments.txt中待查看
    src_path,_ = os.path.split(os.path.realpath(__file__))
    palmPrintsNet.store_revision_info(src_path, log_dir, ' '.join(sys.argv))#存储运行时的版本信息

    np.random.seed(seed=args.seed)
    random.seed(args.seed)
    train_set = palmPrintsNet.get_dataset(args.data_dir)#根据数据目录，得到所有类中每张图片的路径，[ 类1  类2  类3 ...]
    #Total class's number
    nrof_classes = len(train_set)#总共有多少类
...
def parse_arguments(argv):
    ...
    parser.add_argument('--logs_base_dir', type=str,
        help='Directory where to write event logs.', default='logs/palmprint_0111/')
    parser.add_argument('--models_base_dir', type=str,
        help='Directory where to write trained models and checkpoints.', default='src/models/palmprint_0111/')
    parser.add_argument('--data_dir', type=str,
        help='Path to the data directory containing aligned face patches. Multiple directories are separated with colon.',default='data/deeplearningdata/palmprint_v1/train')
